...
# # # def DatasetPreprocess(src, dst, record_path, mode='9th-TypeB', clip_range=[-401.5, 928.5], target_spacing=(1, 1, 1), resample=True, clip=True, target_shape=(256,256,256)):
    
# #     """
# #     :param src: source dir 
# #     :param dst: target dir to save
...
# #             print(f"Total samples found: {len(sample_path_list)}")  # Debugging statement
# #             if resample:
...
# #             for idx, sample_path in enumerate(sample_path_list):
# #                 print(f"Processing sample {idx + 1}/{len(sample_path_list)}: {sample_path}")  # Debugging statement
# #                 img_path = os.path.join(sample_path, 'origin.nii.gz')
# #                 img, spacing, affine_pre = io_.read_nii(img_path)

# #                 # pseudo lumen labeled as 1
...
# #             print(f"Total samples found: {total_num}")  # Debugging statement
# #             for num, sample_path in enumerate(sample_path_list, start=1):
...
# #                 # pseudo lumen labeled as 1 and true lumen labeled as 2
...
# #                 case_idx = 'TBAD_' + os.path.basename(sample_path).split('_')[0]
# #                 saved_path = os.path.join(save_dir, case_idx + '.h5')
# #                 print('Name: {}\nRaw size: {}\nResample size: {}\nPad size: {}\nSave at: {}'.format(case_idx, img.shape, resampled_img.shape, processed_img.shape, saved_path))

# #                 show_graphs_test(img[:,-272:-240,:], resampled_img[:,round(resampled_img.shape[1]/2)-16:round(resampled_img.shape[1]/2)+16,:], processed_img[:,-272:-240,:], label[:,-272:-240,:], processed_mask[:,-272:-240,:], 
# #                                  (25,64), record_path+'/img/'+case_idx+'_img1.png', dim=1)
# #                 show_graphs_test(img[:,:,round(resampled_img.shape[-1]/2)-16:round(resampled_img.shape[-1]/2)+16], resampled_img[:,:,round(resampled_img.shape[-1]/2)-16:round(resampled_img.shape[-1]/2)+16], processed_img[:,:,256-16:256+16], 
# #                                  label[:,:,round(resampled_img.shape[-1]/2)-16:round(resampled_img.shape[-1]/2)+16], processed_mask[:,:,256-16:256+16], (16,80), record_path+'/img/'+case_idx+'_img2.png', dim=2)

# #                 save_to_h5(processed_img, processed_mask, saved_path)
# #                 with open(record_path + '/Processed_ImageTBAD.csv', 'a+', newline='', encoding='gbk') as csv_file:
...
#         if mode == '9th-TypeB':
#             sample_path_list = [os.path.join(src, f) for f in os.listdir(src)]
#             print(f"Total samples found: {len(sample_path_list)}")
#             if resample:
#                 save_dir = os.path.join(save_dir, 'wResample')
#                 os.makedirs(save_dir, exist_ok=True)
#                 print('w Resample...')
...
#             for idx, sample_path in enumerate(sample_path_list):
#                 print(f"Processing sample {idx + 1}/{len(sample_path_list)}: {sample_path}")
#                 img_path = os.path.join(sample_path, 'origin.nii.gz')
#                 img, spacing, affine_pre = io_.read_nii(img_path)
...
#             sample_path_list = [patient for patient in glob.glob(sample_path_dir)][30:]
#             total_num = len(sample_path_list)
#             print(f"Total samples found: {total_num}")
#             for num, sample_path in enumerate(sample_path_list, start=1):
#                 print("[{}/{}]\tload sample from: {}".format(num, total_num, sample_path))
#                 img, spacing, affine_pre = io_.read_nii(sample_path)
...
#                 label_TL, label_FL = label.copy(), label.copy()
#                 label_TL[label_TL != 1] = 0
#                 label_FL[label_FL != 2] = 0
#                 mask = label_FL / 2 + label_TL * 2
...
#     2. Plot the slice in different views
#     3. Crop the target range of CT
#     4. Pad the height and width to the target size
#     4. Resize the total sample to the target size
# '''
...
#     print('Name: {}\nRaw size: {}\nResample size: {}\nPad size: {}\nMask range: {}\nmid slice: {}\nSave at: {}'.format(case_idx, img.shape, resampled_img.shape, processed_img.shape, channel_range, mid_slice, saved_path))
#     show_graphs_test(img[:,-272:-240,:], resampled_img[:,Resample_mid_slice1-16:Resample_mid_slice1+16,:], processed_img[:,-272:-240,:], mask[:,-272:-240,:], processed_mask[:,-272:-240,:], 
#                      (25,64), record_path+'/img/'+case_idx+'_img1.png', dim=1)                
#     show_graphs_test(img[:,:,Resample_mid_slice2-16:Resample_mid_slice2+16], resampled_img[:,:,Resample_mid_slice2-16:Resample_mid_slice2+16], processed_img[:,:,mid_slice-16:mid_slice+16], 
#                      mask[:,:,Resample_mid_slice2-16:Resample_mid_slice2+16], processed_mask[:,:,mid_slice-16:mid_slice+16], 
#                      (16,80), record_path+'/img/'+case_idx+'_img2.png', dim=2)  
...
#         root0 = '/home/amishr17/aryan/new_attempt/data'
#         root1 = 'h5'
#         dst = 'preprocess/TBAD'
#         target_shape = (128, 128, 128)
#         record_path = 'h5'

#         print(f"Source directory: {root1}")
...
#         print("Starting DatasetPreprocess function...")
#         DatasetPreprocess(root0, dst, record_path, mode='ImageTBAD', resample=False, clip=False)
#         print("Finished DatasetPreprocess function.")

#         # Uncomment the following lines if you need to preprocess samples individually
#         # idx = 23
#         # src_list = os.listdir(root0)
#         # src_test = os.path.join(root0, src_list[idx])

#         # csv_file_path = os.path.join(record_path, 'Processed_9th-TypeB.csv')
#         # print(f"Creating CSV file at {csv_file_path}")
#         # with open(csv_file_path, 'w', newline='', encoding='gbk') as csv_file:
#         #     writer = csv.writer(csv_file)
...
def DatasetPreprocess(src, dst, record_path, mode='9th-TypeB', clip_range=[-401.5, 928.5], target_spacing=(1, 1, 1), resample=True, clip=True, target_shape=(64,64,64)):
